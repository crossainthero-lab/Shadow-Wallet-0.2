...
const dom = {
  ...
  buyAmountPrefix: document.getElementById('buyAmountPrefix'),
  buyMaxButton: document.getElementById('buyMaxButton'),
  buyAmountSuffix: document.getElementById('buyAmountSuffix'),
  ...
  swapFromView: document.getElementById('swapFromView'),
  swapToView: document.getElementById('swapToView'),
  ...
  swapMaxButton: document.getElementById('swapMaxButton'),
  swapReceiveLabel: document.getElementById('swapReceiveLabel'),
  swapAvailable: document.getElementById('swapAvailable'),
  sendForm: document.getElementById('sendForm'),
  sendAssetView: document.getElementById('sendAssetView'),
  sendDetailView: document.getElementById('sendDetailView'),
  sendAssetList: document.getElementById('sendAssetList'),
  ...
  sendMaxButton: document.getElementById('sendMaxButton'),
...
function getActionDelay() {
  return Math.floor((0.9 + Math.random() * 2.6) * 1000);
}

function setBusy(isBusy, message = '') {
  state.isBusy = isBusy;
  const controls = document.querySelectorAll('.sheet button, .sheet input, .sheet select');
  controls.forEach((control) => {
    if (control.id === 'closeModalButton' || control.id === 'buyBackButton') {
      return;
    }
  ...
  if (isBusy) {
    dom.sheetToast.textContent = message || 'Processing...';
    dom.sheetToast.classList.remove('hidden');
    dom.sheetToast.classList.add('loading');
  } else {
    dom.sheetToast.classList.remove('loading');
    dom.sheetToast.classList.add('hidden');
  }
}

async function withFakeLoading(message, work) {
  if (state.isBusy) {
    return;
  }
...
function updateBalanceFont(balanceText) {
  ...
  dom.totalBalance.style.fontSize = nextSize;
}
...
function render() {
  ...
  const current = getCurrentAccount();
  const balanceText = formatUsd(summary.totalBalance);
  dom.totalBalance.textContent = balanceText;
  updateBalanceFont(balanceText);
...
function renderSwapPicker() {
  ...
  if (!owned.some((asset) => asset.id === state.selectedSwapFromId)) {
    state.selectedSwapFromId = owned[0]?.id || null;
  }
  if (state.selectedSwapFromId === state.selectedSwapToId) {
...
function getSendAssets() {
  return getCurrentAccount().holdings
    .filter((holding) => holding.amount > 0)
    .map((holding) => ({
      ...TOKENS_BY_ID[holding.tokenId],
      amount: holding.amount
    }));
}

function renderSendAssetPicker() {
  const assets = getSendAssets();
  if (!assets.some((token) => token.id === state.selectedSendTokenId)) {
    state.selectedSendTokenId = assets[0]?.id || null;
  }

  dom.sendAssetList.innerHTML = assets.length
    ? assets.map((token) => `
      <button class="market-item ${token.id === state.selectedSendTokenId ? 'selected-asset' : ''}" type="button" data-send-token-id="${token.id}">
        <div class="market-main">
          ${renderTokenVisual(token, 'market-logo')}
          <div>
            <div class="market-name">${escapeHtml(token.name)}</div>
            <div class="market-meta">${formatAmount(token.amount)} ${token.symbol}</div>
          </div>
        </div>
        <div class="market-values">
          <div class="market-price">${formatUsd((state.prices[token.id]?.usd || 0) * token.amount)}</div>
        </div>
      </button>
    `).join('')
    : '<div class="empty">Add an asset first so there is something to send.</div>';
...
function updateBuyQuote() {
  ...
  dom.buyUnitPills.querySelectorAll('[data-unit]').forEach((button) => {
    ...
    if (button.dataset.unit === 'token') {
      button.textContent = token.symbol;
    }
  });

  const showFiatPrefix = unit === 'usd' || unit === 'aud';
  dom.buyAmountPrefix.classList.toggle('hidden', !showFiatPrefix);
  dom.buyAmountSuffix.classList.toggle('hidden', unit !== 'token');
  dom.buyAmountSuffix.textContent = token.symbol;
  ...
  if (unit === 'aud') {
    ...
    dom.buyQuote.textContent = `Enter amount in ${token.symbol}`;
  } else {
...
function getBuyMaxAmount() {
  const tokenId = state.selectedBuyTokenId || TOKENS[0].id;
  const tokenPrice = state.prices[tokenId];
  const unit = state.buyUnit || 'usd';
  const demoUsdCap = 100000;
  ...
  if (unit === 'aud') {
    return tokenPrice.aud ? demoUsdCap * getAudPerUsd() : 0;
  }
  if (unit === 'token') {
    return demoUsdCap / tokenPrice.usd;
  }
  return demoUsdCap;
}
...
function showSwapFromStage() {
  state.swapView = 'from';
  dom.modalTitle.textContent = 'Swap';
  dom.modalSubtitle.textContent = 'Choose what you already hold.';
  dom.buyBackButton.classList.add('hidden');
  dom.swapFromView.classList.remove('hidden');
  dom.swapToView.classList.add('hidden');
  dom.swapDetailView.classList.add('hidden');
  animateIn(dom.swapFromView, 'view-enter-up');
}

function showSwapToStage() {
  state.swapView = 'to';
  dom.modalTitle.textContent = `Swap ${SWAP_TARGETS_BY_ID[state.selectedSwapFromId]?.symbol || ''}`.trim();
  dom.modalSubtitle.textContent = 'Choose what you want to receive.';
  dom.buyBackButton.classList.remove('hidden');
  dom.swapFromView.classList.add('hidden');
  dom.swapToView.classList.remove('hidden');
  dom.swapDetailView.classList.add('hidden');
  animateIn(dom.swapToView, 'view-enter-side');
}

function showSwapDetailStage() {
  state.swapView = 'detail';
  const fromSymbol = SWAP_TARGETS_BY_ID[state.selectedSwapFromId]?.symbol || 'asset';
  const toSymbol = SWAP_TARGETS_BY_ID[state.selectedSwapToId]?.symbol || 'asset';
  dom.modalTitle.textContent = `Swap ${fromSymbol} to ${toSymbol}`;
  dom.modalSubtitle.textContent = 'Choose the amount to convert.';
  dom.buyBackButton.classList.remove('hidden');
  dom.swapFromView.classList.add('hidden');
  dom.swapToView.classList.add('hidden');
  dom.swapDetailView.classList.remove('hidden');
  animateIn(dom.swapDetailView, 'view-enter-side');
  dom.swapAmountInput.focus();
  updateSwapQuote();
  renderFlowHeroes();
}

function showSendAssetStage() {
  state.sendView = 'asset';
  dom.modalTitle.textContent = 'Send';
  dom.modalSubtitle.textContent = 'Choose which asset to send.';
  dom.buyBackButton.classList.add('hidden');
  dom.sendAssetView.classList.remove('hidden');
  dom.sendDetailView.classList.add('hidden');
  animateIn(dom.sendAssetView, 'view-enter-up');
}

function showSendDetailStage() {
  state.sendView = 'detail';
  const token = TOKENS_BY_ID[state.selectedSendTokenId];
  if (!token) {
    showSendAssetStage();
    return;
  }
  dom.modalTitle.textContent = token ? `Send ${token.symbol}` : 'Send';
  dom.modalSubtitle.textContent = 'Choose a destination and amount.';
  dom.buyBackButton.classList.remove('hidden');
  dom.sendAssetView.classList.add('hidden');
  dom.sendDetailView.classList.remove('hidden');
  animateIn(dom.sendDetailView, 'view-enter-side');
  dom.sendAmountInput.focus();
  updateSendAvailability();
  renderFlowHeroes();
}
...
function renderFlowHeroes() {
  ...
  const sendToken = TOKENS_BY_ID[state.selectedSendTokenId || state.selectedBuyTokenId];
...
function handleFlowBack() {
  if (state.modal === 'buy' && state.buyView === 'detail') {
    showBuyPicker();
    return;
  }
  if (state.modal === 'swap') {
    if (state.swapView === 'detail') {
      showSwapToStage();
      return;
    }
    if (state.swapView === 'to') {
      showSwapFromStage();
    }
    return;
  }
  if (state.modal === 'send' && state.sendView === 'detail') {
    showSendAssetStage();
  }
}
...
async function onBuySubmit(event) {
  ...
  await withFakeLoading('Executing buy...', async () => {
...
async function onSwapSubmit(event) {
  event.preventDefault();
  const fromId = state.selectedSwapFromId;
  const toId = state.selectedSwapToId;
  const amount = Number(dom.swapAmountInput.value);
...
async function copyAddress() {
  ...
  try {
    ...
    } else if (globalThis.ShadowAndroid?.copyText) {
      globalThis.ShadowAndroid.copyText(address);
    } else {
...
async function createAccount(event) {
  ...
  await withFakeLoading('Creating account...', async () => {
...
async function wipeAllAccounts() {
  await withFakeLoading('Wiping all accounts...', async () => {
    ...
    state.sendView = 'asset';
    state.selectedSendTokenId = null;
...
dom.buyBackButton.addEventListener('click', handleFlowBack);
...
dom.sendAssetList.addEventListener('click', (event) => {
  if (!(event.target instanceof Element)) return;
  const button = event.target.closest('[data-send-token-id]');
  if (!button) return;
  state.selectedSendTokenId = button.dataset.sendTokenId;
  renderSendAssetPicker();
  showSendDetailStage();
});
